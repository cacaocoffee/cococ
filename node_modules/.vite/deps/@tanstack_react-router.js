...
function createRouterStores(initialState, config) {
  ...
  const hasPendingMatches = createReadonlyStore(() => matchesId.state.some((matchId) => {
    var _a;
    return ((_a = activeMatchStoresById.get(matchId)) == null ? void 0 : _a.state.status) === "pending";
  }));
  const matchRouteReactivity = createReadonlyStore(() => {
    var _a;
    return {
      locationHref: location.state.href,
      resolvedLocationHref: (_a = resolvedLocation.state) == null ? void 0 : _a.href,
      status: status.state
    };
  });
  ...
  function setActiveMatches(nextMatches) {
    reconcileMatchPool(nextMatches, activeMatchStoresById, matchesId, createMutableStore, batch2);
  }
  function setPendingMatches(nextMatches) {
    reconcileMatchPool(nextMatches, pendingMatchStoresById, pendingMatchesId, createMutableStore, batch2);
  }
  function setCachedMatches(nextMatches) {
    reconcileMatchPool(nextMatches, cachedMatchStoresById, cachedMatchesId, createMutableStore, batch2);
  }
  return store;
}
...
var hasForcePendingActiveMatch = (router) => {
  return router.stores.matchesId.state.some((matchId) => {
    var _a;
    return (_a = router.stores.activeMatchStoresById.get(matchId)) == null ? void 0 : _a.state._forcePending;
  });
};
...
var loadRouteMatch = async (inner, matchPromises, index) => {
  ...
  if (shouldSkipLoader(inner, matchId)) {
    ...
    const previousRouteMatchId = ((_a = activeIdAtIndex && inner.router.stores.activeMatchStoresById.get(activeIdAtIndex) || null) == null ? void 0 : _a.routeId) === routeId ? activeIdAtIndex : (_b = inner.router.stores.activeMatchesSnapshot.state.find((d) => d.routeId === routeId)) == null ? void 0 : _b.id;
    const preload = resolvePreload(inner, matchId);
...
function createBrowserHistory(opts) {
  ...
  const queueHistoryAction = (type, destHref, state) => {
    ...
    if (!scheduled) scheduled = Promise.resolve().then(() => flush3());
  };
...
var RouterCore = class {
  ...
  constructor(options, getStoreConfig) {
    ...
    this.cancelMatches = () => {
      ...
      this.stores.matchesId.state.forEach((matchId) => {
        ...
        const match = (_a = this.stores.activeMatchStoresById.get(matchId)) == null ? void 0 : _a.state;
        if (!match) return;
      ...
      loadPromise = new Promise((resolve) => {
        this.startTransition(async () => {
          ...
          try {
            ...
            const nextStatusCode = redirect2 ? redirect2.status : notFound2 ? 404 : this.stores.activeMatchesSnapshot.state.some((d) => d.status === "error") ? 500 : 200;
            this.batch(() => {
    ...
    this.getMatch = (matchId) => {
      var _a, _b, _c;
      return ((_a = this.stores.cachedMatchStoresById.get(matchId)) == null ? void 0 : _a.state) ?? ((_b = this.stores.pendingMatchStoresById.get(matchId)) == null ? void 0 : _b.state) ?? ((_c = this.stores.activeMatchStoresById.get(matchId)) == null ? void 0 : _c.state);
    };
    ...
    this.matchRoute = (location, opts) => {
      ...
      if ((opts == null ? void 0 : opts.pending) && this.stores.status.state !== "pending") return false;
      const baseLocation = ((opts == null ? void 0 : opts.pending) === void 0 ? !this.stores.isLoading.state : opts.pending) ? this.latestLocation : this.stores.resolvedLocation.state || this.stores.location.state;
  ...
  */
  matchRoutesLightweight(location) {
    ...
    const lastStateMatch = lastStateMatchId && ((_a = this.stores.activeMatchStoresById.get(lastStateMatchId)) == null ? void 0 : _a.state);
...
var { link, unlink, propagate, checkDirty, shallowPropagate } = createReactiveSystem({
  ...
  notify(effect2) {
    ...
    do {
      ...
      effect2.flags &= -3;
      effect2 = (_a = effect2.subs) == null ? void 0 : _a.sub;
...
function useMatch(opts) {
  ...
  if (isServer ?? router.isServer) {
    const match = matchStore == null ? void 0 : matchStore.state;
...
function useLinkProps(options, forwardedRef) {
  ...
  const { activeProps, inactiveProps, activeOptions, to, preload: userPreload, preloadDelay: userPreloadDelay, hashScrollIntoView, replace, startTransition: startTransition2, resetScroll, viewTransition, children, target, disabled, style, className, onClick, onBlur, onFocus, onMouseEnter, onMouseLeave, onTouchStart, ignoreBlocker, params: _params, search: _search, hash: _hash, state: _state, mask: _mask, reloadDocument: _reloadDocument, unsafeRelative: _unsafeRelative, from: _from, _fromLocation, ...propsSafeToSpread } = options;
  if (_isServer) {
...
function CatchNotFound(props) {
  const router = useRouter();
  if (isServer ?? router.isServer) {
    const resetKey2 = `not-found-${router.stores.location.state.pathname}-${router.stores.status.state}`;
...
function ScrollRestoration() {
  ...
  return (0, import_jsx_runtime10.jsx)(ScriptOnce, { children: `(${restoreScroll.toString()})(${escapeHtml(JSON.stringify(restoreScrollOptions))})` });
}
...
var Match = React$111.memo(function MatchImpl({ matchId }) {
  ...
  const router = useRouter();
  if (isServer ?? router.isServer) {
    const match2 = (_a = router.stores.activeMatchStoresById.get(matchId)) == null ? void 0 : _a.state;
    if (!match2) {
...
function MatchView({ router, matchId, resetKey, matchState }) {
  ...
  }), matchState.parentRouteId === rootRouteId && router.options.scrollRestoration ? (0, import_jsx_runtime11.jsxs)(import_jsx_runtime11.Fragment, { children: [(0, import_jsx_runtime11.jsx)(OnRendered, {}), (0, import_jsx_runtime11.jsx)(ScrollRestoration, {})] }) : null] });
}
function OnRendered() {
  const router = useRouter();
  const prevLocationRef = React$111.useRef(void 0);
...
var MatchInner = React$111.memo(function MatchInnerImpl({ matchId }) {
  ...
  if (isServer ?? router.isServer) {
    ...
    if (match2._displayPending) throw (_c = router.getMatch(match2.id)) == null ? void 0 : _c._nonReactive.displayPendingPromise;
    if (match2._forcePending) throw (_d = router.getMatch(match2.id)) == null ? void 0 : _d._nonReactive.minPendingPromise;
    if (match2.status === "pending") throw (_e = router.getMatch(match2.id)) == null ? void 0 : _e._nonReactive.loadPromise;
    ...
    if (match2.status === "redirected") {
      ...
      throw (_f = router.getMatch(match2.id)) == null ? void 0 : _f._nonReactive.loadPromise;
    }
  ...
  if (match._displayPending) throw (_g = router.getMatch(match.id)) == null ? void 0 : _g._nonReactive.displayPendingPromise;
  if (match._forcePending) throw (_h = router.getMatch(match.id)) == null ? void 0 : _h._nonReactive.minPendingPromise;
  if (match.status === "pending") {
    ...
    throw (_i = router.getMatch(match.id)) == null ? void 0 : _i._nonReactive.loadPromise;
  }
  ...
  if (match.status === "redirected") {
    ...
    throw (_j = router.getMatch(match.id)) == null ? void 0 : _j._nonReactive.loadPromise;
  }
...
function useElementScrollRestoration(options) {
  ...
  return (_c = (_b = scrollRestorationCache) == null ? void 0 : _b.state[restoreKey]) == null ? void 0 : _c[elementSelector];
}
...
function useCanGoBack() {
  const router = useRouter();
  if (isServer ?? router.isServer) return router.stores.location.state.state.__TSR_index !== 0;
...
function buildTagsFromMatches(router, nonce, matches) {
  ...
  resultMeta.reverse();
  const constructedLinks = matches.map((match) => match.links).filter(Boolean).flat(1).map((link3) => ({
    tag: "link",
    attrs: {
      ...link3,
      nonce
...
var useTags = () => {
  ...
  const links = useStore(router.stores.activeMatchesSnapshot, (matches) => {
    var _a2;
    const constructed = matches.map((match) => match.links).filter(Boolean).flat(1).map((link3) => ({
      tag: "link",
      attrs: {
        ...link3,
        nonce
...
var Scripts = () => {
  ...
  if (isServer ?? router.isServer) {
    const assetScripts2 = getAssetScripts(router.stores.activeMatchesSnapshot.state);
    return renderScripts(router, getScripts(router.stores.activeMatchesSnapshot.state), assetScripts2);
  }
