...
import {
  useScheduleList,
} from "@/domain/schedule/schedule-query-options";
import {
  useAddSchedule,
  useUpdateSchedule,
  useDeleteSchedule,
...
import type { ScheduleItem, ScheduleType } from "@/domain/schedule/schedule-dto";
import {
  inputCss,
  labelCss,
  formCardCss,
...
interface ScheduleFormProps {
  ...
  onSave: (data: Omit<ScheduleItem, "id" | "createdAt">) => void;
  onCancel: () => void;
  onAlert: (msg: string) => void;
}

function ScheduleForm({ initial = EMPTY_FORM, archiveOptions, onSave, onCancel, onAlert }: ScheduleFormProps) {
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      type: f.type,
      archiveId: f.archiveId ? f.archiveId : null,
    });
  };
...
export default function ScheduleTab() {
  ...
  const [mode, setMode] = useState<"create" | ScheduleItem | null>(null);
    ...
    <div>
      ...
      {mode && mode !== "create" && (
        <div style={{ marginBottom: "24px" }}>
          ...
          <ScheduleForm
            initial={toForm(mode as ScheduleItem)}
            archiveOptions={archiveOptions}
            onAlert={(msg) => openAlert({ title: msg, description: "", type: "error" })}
            onSave={(d) =>
              updateMutation.mutate(
                { id: (mode as ScheduleItem).id, data: d },
