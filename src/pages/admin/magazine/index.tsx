...
// Form state for MagazineForm (tags as string)
interface MagazineFormState {
  title: string;
  author: string;
  date: string;
  readTime: string;
...
function MagazineForm({ initial = EMPTY_MAG as MagazineFormState, onSave, onCancel, onAlert }: MagazineFormProps) {
  ...
  const setV = (k: keyof MagazineFormState) => (v: unknown) => setF((p) => ({ ...p, [k]: v }));

  const magType = f.magazineType || "blog";
  const setMagType = (t: string) => setF((p) => ({ ...p, magazineType: t }));

  // 블로그 블록
  const blocks: ContentBlock[] = Array.isArray(f.content) ? f.content : [];
  ...
  // 카드뉴스 — 인스타그램 URL 목록
  const instagramUrls: string[] = Array.isArray(f.instagramUrls) ? f.instagramUrls : [];
  const setInstagramUrls = (next: string[]) => setF((p) => ({ ...p, instagramUrls: next }));
  const addInstagramUrl = () => setInstagramUrls([...instagramUrls, ""]);
  const removeInstagramUrl = (i: number) => setInstagramUrls(instagramUrls.filter((_, idx) => idx !== i));
  const moveInstagramUrl = (i: number, dir: number) => {
  ...
  const updateInstagramUrl = (i: number, val: string) => setInstagramUrls(instagramUrls.map((s, idx) => (idx === i ? val : s)));
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const json = await res.json() as { status: string; data?: { image?: { url?: string } } };
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      content: magType === "blog" ? blocks.map((b) => ({ heading: (b as TextBlock).heading ?? "", body: (b as TextBlock).body ?? "" })) : [],
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            {instagramUrls.map((url, i) => (
              <div key={i} className={slideRowCss}>
                ...
                <input
                  value={url}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateInstagramUrl(i, e.target.value)}
...
// ─── MagazineTab ──────────────────────────────────────────────
const toForm = (item: MagazineItem): MagazineFormState => ({
  magazineType: "blog",
  instagramUrls: [],
  title: item.title,
  author: item.author,
  ...
  ...(item.magazineType ? { magazineType: item.magazineType } : {}),
  ...(item.instagramUrls ? { instagramUrls: item.instagramUrls } : {}),
});
